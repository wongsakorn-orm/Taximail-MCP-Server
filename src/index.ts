...
if (!TAXIMAIL_API_KEY || !TAXIMAIL_SECRET_KEY) {
  console.error(
    "The TAXIMAIL_API_KEY or TAXIMAIL_SECRET_KEY environment variable is not set."
  );
  process.exit(1);
}
...
server.tool(
  ...
  "A transactional email is a kind of email that is triggered by a user action on an app. It is sent to facilitate an agreed-upon transaction between the sender and the recipient.",
  {
    ...
    to: z
      .string()
      .describe(
        "This parameter determines the destination phone number for your SMS message. It is MSIDSN of the recipient that the message will be sent to. Format this number with a country code, e.g. 66175551212 (66 is TH code)."
      ),
    text: z
      .string()
      .describe(
        "The text parameter includes the full text of the message you want to send. It is the message content in which you can include any URL."
      ),
  },
  async ({ from, to, text }) => {
